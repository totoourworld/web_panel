...
(function($) {
    ...
    $.documentCenter = function(el) {
        ...
        el.css({
            position: 'absolute',
            top: (Math.max((document.documentElement.clientHeight - el.height()) / 2 + window.scrollY, 0) + 20) + 'px',
            left: (Math.max((document.documentElement.clientWidth - el.width()) / 2 + document.documentElement.scrollLeft, 0) + 120) + 'px',
        })
    };
    ...
    var opening = false;
    $.ShowIfrmDailogRev = function(url, options) {
        if (opening) {
            return
        }
        opening = true;
