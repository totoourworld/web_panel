; (function($j) {
    ...
    $j.browser.msie8 = $j.browser.msie && /msie 8\.0/i.test(userAgent);
    $j.browser.msie7 = $j.browser.msie && /msie 7\.0/i.test(userAgent);
    $j.browser.msie6 = !$j.browser.msie8 && !$j.browser.msie7 && $j.browser.msie && /msie 6\.0/i.test(userAgent);
    ...
    $j.fn.datepicker = function(o) {
        ...
        if (cp.length == 0) {
            ...
            cp = $j("#BBIT_DP_CONTAINER");

            initevents();
        }
        function initevents() {
            //1 today btn;
            $j("#BBIT-DP-TODAY").click(returntoday);
            cp.click(returnfalse);
            $j("#BBIT_DP_INNER tbody").click(tbhandler);
            $j("#BBIT_DP_LEFTBTN").click(prevm);
            $j("#BBIT_DP_RIGHTBTN").click(nextm);
            $j("#BBIT_DP_YMBTN").click(showym);
            $j("#BBIT-DP-MP").click(mpclick).dblclick(mpdblclick);

            $j("#BBIT-DP-MP-PREV").click(mpprevy);
            $j("#BBIT-DP-MP-NEXT").click(mpnexty);
            $j("#BBIT-DP-MP-OKBTN").click(mpok);
            $j("#BBIT-DP-MP-CANCELBTN").click(mpcancel);
        }
        function mpcancel() {
            $j("#BBIT-DP-MP").animate({ top: -193 }, { duration: 200, complete: function() { $j("#BBIT-DP-MP").hide(); } });
            return false;
        }
        function mpok() {
            def.Year = def.cy;
        ...
        function rryear(y) {
            ...
            $j("#BBIT-DP-MP td.bbit-dp-mp-year").each(function(i) {
                if (def.Year == ar[i]) {
                    $j(this).addClass("bbit-dp-mp-sel");
                }
                else {
                    $j(this).removeClass("bbit-dp-mp-sel");
                }
                $j(this).html("<a href='javascript:void(0);'>" + ar[i] + "</a>").attr("xyear", ar[i]);
            });
        }
        function mpdblclick(e) {
            ...
            if ($j(td).hasClass("bbit-dp-mp-month") || $j(td).hasClass("bbit-dp-mp-year")) {
                mpok(e);
            }
            return false;
        }
        function mpclick(e) {
            ...
            if ($j(td).hasClass("bbit-dp-mp-month")) {
                if (!$j(td).hasClass("bbit-dp-mp-sel")) {
                    var ctd = panel.find("td.bbit-dp-mp-month.bbit-dp-mp-sel");
                    if (ctd.length > 0) {
                        ctd.removeClass("bbit-dp-mp-sel");
                    }
                    $j(td).addClass("bbit-dp-mp-sel")
                    def.cm = parseInt($j(td).attr("xmonth"));
                }
            }
            if ($j(td).hasClass("bbit-dp-mp-year")) {
                if (!$j(td).hasClass("bbit-dp-mp-sel")) {
                    ...
                    $j(td).addClass("bbit-dp-mp-sel")
                    def.cy = parseInt($j(td).attr("xyear"));
                }
            }
            return false;
        }
        function showym() {
            ...
            var m = def.Month - 1;
            def.cm = m;
            var ms = $j("#BBIT-DP-MP td.bbit-dp-mp-month");
            for (var i = ms.length - 1; i >= 0; i--) {
                var ch = $j(ms[i]).attr("xmonth");
                if (ch == m) {
                    $j(ms[i]).addClass("bbit-dp-mp-sel");
                }
                else {
                    $j(ms[i]).removeClass("bbit-dp-mp-sel");
                }
            }
            rryear(y);
        ...
        function tbhandler(e) {
            ...
            var $td = $j(td);
            if (!$j(td).hasClass("bbit-dp-disabled")) {
                var s = $td.attr("xdate");              
        ...
        function writecb() {
            var tb = $j("#BBIT_DP_INNER tbody");
            $j("#BBIT_DP_YMBTN").html(def.monthName[def.Month - 1] + def.monthp + " " + def.Year);
            ...
            for (var i = 1; i <= 42; i++) {
                ...
                else
				{
					...
					{
						 tdc.push("bbit-dp-active st-bg-weekend");
					}
					else
					{
        ...
        return $j(this).each(function() {
            ...
            var obj = $j(this).addClass("bbit-dp-input");
            var picker = $j(def.picker);
            def.showtarget == null && obj.after(picker);
            picker.click(function(e) {
                var isshow = $j(this).attr("isshow");
                //hide it initially
                var me = $j(this);
                ...
                if (def.applyrule && $j.isFunction(def.applyrule)) {
                ...
                //cp.show();
                $j(this).attr("isshow", "1");

                $j(document).one("click", function(e) {
                    me.attr("isshow", "0");
