...
(function($j) {
    $j.fn.DhoverClass = function(className) {
        return $j(this).hover(function() { $j(this).addClass(className); }, function() { $j(this).removeClass(className); });
    }
    function getDulyOffset(target, w, h) {
        var pos = target.offset();
        var height = target.outerHeight();
        var newpos = { left: pos.left, top: pos.top + height - 1 }
        var bw = $j(document).width();
        var bh = $j(document).height();
        if ((newpos.left + w) >= bw) {
            newpos.left = bw - w - 2;
        }
    ...
    $j.fn.colorselect = function(option) {        
        ...
        var leftdiv = $j("<div/>").addClass("leftdiv").attr("id", leftId).appendTo(me);
        ...
        // html.push('</div>');
        var div = $j("<div />").addClass("x-color-palette").css({
            position: "absolute",
            "z-index": "999",
            "overflow": "auto",
            width: options.width,
        ...
        me.click(function() {
            ...
            div.css(pos);
            div.show();
            $j(document).one("click", function(event) { div.hide(); });
            return false;
        });
        ...
        if (options.hiddenid != null && options.hiddenid != "") {
            $j("#" + options.hiddenid).val(options.index);
        }
        /*     */
